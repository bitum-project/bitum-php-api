<?php namespace Bitum\Tests;

use GuzzleHttp\Client;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
use Bitum\Rate;
use Bitum\Rate\CoinMarketCap;
...
class RateTest extends TestCase
{
    public function test_get_price()
    {
        ...
        $amountBITUM = $rate->convertToCrypto(7.09);
        $this->assertTrue(is_numeric($amountBITUM));
        $this->assertEquals(0.00991572, $rate->getToRate(), '', 0.00000001);
        $this->assertEquals(100.8499, $rate->getFromRate(), '', 0.00000001);
        $this->assertEquals(0.0703025, $amountBITUM, '', 0.00000001);
        $this->assertEquals(7.09, $rate->convertToFiat($amountBITUM), '', 0.0001);

        $amountBITUM = $rate->convertToCrypto(10.137);
        $this->assertTrue(is_numeric($amountBITUM));
        $this->assertEquals(0.00991572, $rate->getToRate(), '', 0.00000001);
        $this->assertEquals(100.8499, $rate->getFromRate(), '', 0.00000001);
        $this->assertEquals(0.10051572, $amountBITUM, '', 0.00000001);
        $this->assertEquals(10.137, $rate->convertToFiat($amountBITUM), '', 0.0001);
    }
